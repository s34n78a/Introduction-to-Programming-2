...
// TODO: forward-declare emit helpers, allocator, symbol helpers, validation, codeGen
...
// TODO: findSym — returns index of name in table, or -1 if not found
int findSym(const char *n) {
    int i;
    for (i = 0; i < sbcount; i++)
...
// TODO: addSym — appends a new undefined variable to the symbol table
int addSym(const char *n) {
...
============================================================================================*/
// TODO: emit — writes formatted assembly into outBuf instead of printing directly
void emit(const char *fmt, ...) {
    va_list ap; 
    char tmp[512];
    va_start(ap, fmt); 
...
// TODO: emit helpers — one per instruction type, no commas in output
void emitMOV_RC(int d, int c) { 
    emit("MOV r%d %d\n",   d, c); 
}
